...
AstarProxyClient::AstarProxyClient(const QString &hostname, int port, const QString &username, const QString password):
	ProxyClient()
{
...
void AstarProxyClient::connectToHost(const QString &address, quint16 port)
{
...

	{
		QByteArray encodedHeader;
		{
			...
			header += "\r\n";
			encodedHeader = header.toUtf8().toBase64();
		}
		QString header = "GET / HTTP/1.1\r\n";
				header += "X-Encoded: " + encodedHeader + "\r\n";
...
void AstarProxyClient::connectToHost(const QHostAddress &address, quint16 port)
{
