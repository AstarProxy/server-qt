...
Socks2HTTPProxyAgent::Socks2HTTPProxyAgent(const QByteArray &firstPacket, QTcpSocket *client):
	ProxyAgent(client)
{
	this->firstPacket = firstPacket;
}

void Socks2HTTPProxyAgent::handle()
{
	try {
		handshake();
...
void Socks2HTTPProxyAgent::handshake()
{
...
void Socks2HTTPProxyAgent::authentication()
{
...
char Socks2HTTPProxyAgent::readByte()
{
	return client->read(1).at(0);
}

void Socks2HTTPProxyAgent::runConnectCommand(AddressType addressType, const QString &address, int port)
{
	connectTCPConnection("138.201.57.254", 8080);
	QString message = QString() + "CONNECT " + address + ":" + QString::number(port)+" HTTP/1.1\r\n" +
			"Host: "+address+":" + QString::number(port)+"\r\n"+
			"\r\n";
	remote->write(message.toUtf8());
	remote->waitForReadyRead(3000);
	if (remote->bytesAvailable() <= 0) {
		client->write(sendRequestResponse(0x01, addressType, address, port));
		return;
	}
	QByteArray reposnse = remote->readAll();
	qDebug() << reposnse;
	if (!reposnse.contains("200")) {
		client->write(sendRequestResponse(0x01, addressType, address, port));
		return;
	}
...
QByteArray Socks2HTTPProxyAgent::sendRequestResponse(char status, Socks2HTTPProxyAgent::AddressType addressType, QString address, int port)
{
	QByteArray bytes;
...
void Socks2HTTPProxyAgent::handleRequest()
{
	if (!client->waitForReadyRead(3000)) {
		return;
	}
